...
/* ── sidebar ── */
[data-testid="stSidebar"]                    { background: #4A0000 !important; border-right: 3px solid #3A0000; }
...
[data-testid="stSidebar"] hr                 { border-color: #7A2020; }

/* ── typography ── */
h1 { color: #3A0000 !important; font-size: 1.65rem !important; }
...
.stButton > button:hover {
    ...
    box-shadow: 0 4px 14px rgba(107,0,0,.45) !important;
    transform: translateY(-1px) !important;
}
.stDownloadButton > button {
    background: linear-gradient(135deg,#0FA958 0%,#0d8a47 100%) !important;
    color:#fff !important; border:none !important; border-radius:8px !important;
    padding:10px 26px !important; font-weight:700 !important; font-size:1.05rem !important;
}
.stDownloadButton > button:hover { filter: brightness(1.1); }

/* ── cards ── */
.card {
    background:#FFFFFF; border-radius:12px; padding:20px 26px;
    box-shadow:0 2px 14px rgba(107,0,0,.08); margin-bottom:18px;
}
.card-sm {
    background:#FFFFFF; border-radius:10px; padding:14px 18px;
    box-shadow:0 1px 8px rgba(107,0,0,.07); margin-bottom:12px;
}

/* ── KPI cards ── */
.kpi { background:#FFF; border-radius:12px; padding:16px 14px;
       box-shadow:0 2px 12px rgba(107,0,0,.10);
       border-left:4px solid #6B0000; text-align:center; margin-bottom:10px; }
.kpi.g  { border-left-color:#0FA958; }
.kpi.o  { border-left-color:#FF5A1F; }
.kpi.r  { border-left-color:#C91818; }
.kpi .v { font-size:1.4rem; font-weight:800; color:#3A0000; line-height:1.1; }
.kpi .l { font-size:.70rem; color:#6B6B6B; text-transform:uppercase;
          letter-spacing:.06em; margin-top:4px; }

/* ── section title ── */
.stitle { color:#6B0000; font-size:1.05rem; font-weight:700;
          border-bottom:2px solid #FF5A1F; padding-bottom:5px; margin:16px 0 12px; }
...
/* ── alerts ── */
.alert-row { background:#FFF3F0; border-left:4px solid #C91818;
...
with st.sidebar:
    ...
    page = st.radio("Nawigacja", PAGES, label_visibility="hidden")

    st.markdown("---")
    base_ok   = st.session_state.get("uf_base") is not None
    result_ok = st.session_state.get("result")  is not None
    st.markdown(
        f"**Baza:** {'<span class=\"bok\">✓ OK</span>' if base_ok else '<span class=\"bmiss\">✗ brak</span>'}",
        unsafe_allow_html=True,
    )
    st.markdown(
        f"**Dane:** {'<span class=\"bok\">gotowe</span>' if result_ok else '<span class=\"bmiss\">nie obliczone</span>'}",
        unsafe_allow_html=True,
    )
    st.markdown("---")
    st.markdown(
        "<small style='color:#FF8C66;'>v3.0 · Postkalkulacja</small>",
        unsafe_allow_html=True,
    )
